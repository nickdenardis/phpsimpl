...
class Field
{
     ...
     * @return \Simpl\Field
     */
    public function __construct(\Simpl\Validate $validator)
    {
    ...
    public function Form($options='', $config='', $multi=false, $prefix='')
    {
        ...
        if ($multi) {
            $this->Set('multi', $this->Get('multi')+1);
        }
        ...
        } elseif (is_array($this->Get('options')) && count($this->Get('options')) > 0) {
            // Multi Options
