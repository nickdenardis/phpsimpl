...
define("TIME_ZONE", date('T'));

/**
 * FeedItem is a part of a FeedCreator feed.
 */
...
/**
 * FeedImage may be added to a FeedCreator feed.
 */
...
/**
 * Feed lets you choose during runtime which format to build.
 */
class Feed extends FeedCreator
{
    ...
    public function _setFormat($format)
    {
        ...
        // overwrite encoding if it was specifically set by the cleint code
     ...
     * @param    string    format    format the feed should comply to. Valid values are: "PIE0.1", "mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM0.3", "HTML", "JS"
     ...
     * @since 1.4
     *
     * @param	string	format	format the feed should comply to. Valid values are: "PIE0.1" (deprecated), "mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM", "ATOM0.3", "HTML", "JS"
     ...
     * To effectively use caching, you should create the FeedCreator object and call this method before anything else, especially before you do the time consuming task to build the feed
     *
     * @param   string   format   format the feed should comply to. Valid values are: "PIE0.1" (deprecated), "mbox", "RSS0.91", "RSS1.0", "RSS2.0", "OPML", "ATOM0.3".
...
class UniversalFeedCreator extends Feed
{
}
...
class FeedCreator extends HtmlDescribable
{
    ...
    /**
     * Any additional elements to include as an assiciated array. All $key => $value pairs will be included unencoded in the feed in the form
     * <$key>$value</$key>
     * Again: No encoding will be used! This means you can invalidate or enhance the feed if $value contains markup. This may be abused to embed tags not implemented by the FeedCreator class used.
     */
    public $additionalElements = array();
    ...
    public function _createNSDeclaration($items)
    {
        $ns = '';
        $declared = array();
        ...
        if (is_array($items)) {
            foreach ($items as $itemkey=>$item) {
                if (is_array($item->additionalElements)) {
                    foreach ($item->additionalElements as $key=>$value) {
                        if (!in_array($key, $declared)) {
                            $ns .= ' xmlns:' . $key .'="http://wayne.edu/feeds/"';
                            $declared[] = $key;
                        }
                    }
                }
...
class RSSCreator091 extends FeedCreator
{
    ...
    public function createFeed()
    {
        ...
        $feed.= $this->_createGeneratorComment();
        $feed.= $this->_createStylesheetReferences();
        $feed.= "<rss version=\"".$this->RSSVersion."\" xmlns:atom=\"http://www.w3.org/2005/Atom\"" . $this->_createNSDeclaration($this->items) . ">\n";
        $feed.= "    <channel>\n";
        $feed.= "        <title>".FeedCreator::iTrunc(htmlspecialchars(str_replace('&', 'and', $this->title)), 100)."</title>\n";
        $this->descriptionTruncSize = 500;
        $feed.= "        <description>".$this->getDescription()."</description>\n";
        $feed.= "		<atom:link href=\"" . $this->link . "\" rel=\"self\" type=\"application/rss+xml\" />\n";
        $feed.= "        <link>".$this->link."</link>\n";
        $now = new FeedDate();
        $feed.= "        <lastBuildDate>".htmlspecialchars($now->rfc822())."</lastBuildDate>\n";
        $feed.= "        <generator>PHPSimpl</generator>\n";
        ...
        for ($i=0;$i<count($this->items);$i++) {
            ...
            */
            ...
            if ($this->items[$i]->guid!="") {
                $feed.= "            <guid isPermaLink=\"true\">".htmlspecialchars($this->items[$i]->guid)."</guid>\n";
            } elseif ($this->items[$i]->link!="") {
                $feed.= "            <guid isPermaLink=\"true\">".htmlspecialchars($this->items[$i]->link)."</guid>\n";
            }
            $feed.= $this->_createAdditionalElements($this->items[$i]->additionalElements, "        ");
            $feed.= "        </item>\n";
        }
        ...
        for ($i=0;$i<count($this->items);$i++) {
            ...
            if ($this->items[$i]->title) {
                if ($this->items[$i]->link) {
                    $feedArray[] = "<div class='".$this->stylePrefix."item_title'><a href='".$this->items[$i]->link."' class='".$this->stylePrefix.
