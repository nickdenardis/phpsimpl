...
class DbTemplate extends Form
{
    ...
    public function GetList($fields = array(), $order_by = '', $sort = '', $offset = '', $limit = '')
    {
        ...
        foreach ($this->join_class as $key => $class) {
            ...
            foreach ($values as $name => $value) {
                $where .= ((string)$value != '') ? '`' . $class->database . '`.`' . $class->table . '`.' . $name . ' ' . (($class->Get('type',
                            $name) == 'string' || $class->Get('type',
                            $name) == 'blob') ? 'LIKE' : '=') . ' \'' . $this->db_link->Prepare($value) . '\' AND ' : '';

                // Create the search
                $search .= ($class->search != '') ? '`' . $class->database . '`.`' . $class->table . '`.' . $name . ' ' . (($class->Get('type',
                            $name) == 'string' || $class->Get('type',
                            $name) == 'blob') ? 'LIKE \'%' . $this->db_link->Prepare($class->search) . '%\'' : ' = \'' . $this->db_link->Prepare($class->search) . '\'') . ' OR ' : '';
            }
        ...
        $query .= ($where != '' || $this->conditions != '') ? ' WHERE ' . (($this->conditions != '') ? $where . $this->conditions : substr($where,
                0, -5)) : '';
    ...
    public function Search($keywords, $search_fields, $return_fields)
    {
        ...
        // Setup the return fields
        if (!$this->isMultiArray($return_fields)) {
        ...
        $query = 'SELECT ' . substr($return, 0,
                -2) . ' FROM `' . $this->database . '`.`' . $this->table . '`' . $join . ' WHERE ' . $parts;
        $query .= ($this->group_by != '') ? ' GROUP BY ' . $this->group_by : '';
        $result = $this->db_link->Query($query, $this->database);
    ...
    public function Move($direction, $options = array())
    {
        ...
        foreach ($options as $field => $class) {
            ...
            if (in_array($field, $fields) && is_object($class)) {
                ...
                // Make the query
                $query = 'SELECT `' . $class->primary . '`,`' . $field . '` FROM `' . $this->table . '` WHERE ' . substr($q_filter,
                        0, -4) . ' ORDER BY `' . $field . '` ' . (($direction == 'up') ? 'DESC ' : 'ASC ') . 'LIMIT 1';
                $result = $this->db_link->Query($query, $this->database);
                ...
                if ($this->db_link->NumRows($result) == 1) {
                    ...
                    if ($new_order[$field] != $this->GetValue($field)) {
                        ...
                        $this->db_link->Perform($class->table, $oldArray, 'update',
                            '`' . $class->primary . '`=\'' . $new_order[$class->primary] . '\'');

                        // Update the New one
                        $newArray = array($field => $new_order[$field]);
                        $this->db_link->Perform($this->table, $newArray, 'update',
    ...
    public function Form(
        ...
        // Start the fieldset
        echo '<fieldset id="table_' . $this->table . (($multi) ? '_' . $this->multi : '') . '"><legend>' . ucfirst(str_replace('_',
                ' ', $this->table)) . ' Information</legend>' . "\n";

        // Show the fields
        foreach ($this->display as $field) {
            $this->fields[$field]->Form((isset($options[$field])) ? $options[$field] : '',
                (isset($config[$field])) ? $config[$field] : '', $multi, $this->prefix);
        }
        ...
        if ($count > 0) {
            ...
            if (is_array($display)) {
                foreach ($display as $key => $field) {
                    $show[$field] = ($this->IsField($field)) ? $this->fields[$field]->Label() : ucfirst(str_replace('_',
                        ' ', $field));
                }
            } else {
            ...
            foreach ($this->results as $key => $field) {
                ...
                foreach ($show as $name => $value) {
                    ...
                    if (isset($options[$name]) && is_array($options[$name])) {
                        ...
                        if (isset($options[$name]) && function_exists($options[$name])) {
                            $func = $options[$name];
                            $str = $func($str);
                        } else {
                            if (isset($options[$name]) && $options[$name] == 'move') {
    ...
    public function Join($join_class, $join_on, $type = 'INNER')
    {
        ...
        if (is_object($join_class)) {
            ...
            $this->join_type[] = $type;
            $this->join_on[] = (stripos($join_on, '=') !== false) ? explode('=', $join_on, 2) : array(
                $join_on,
                $join_on
            );
            return true;
    ...
    private function ParseTable()
    {
        ...
        $result = $this->db_link->Query($query, $this->database, false);
        $result->getColumnMeta(0);
        // Loop through all the fields
        foreach ($result as $id => $field) {
            // Setup each field information
            $tmpField = new Field(new Validate);
            $tmpField->Set('name', $field['Field']);
            $tmpField->Set('type', $this->db_link->ParseType($field['Type']));
            $tmpField->Set('length', $this->db_link->ParseLength($field['Type']));


            $tmpField->Set('validate', $this->ValidType($field));
            $tmpField->Set('primary', (int)($field['Key'] == 'PRI'));
            $tmpField->Set('display', ($id + 1));
            ...
            $this->fields[$field['Field']] = $tmpField;
        }
...
*/
     ...
     * @return bool
     */
    public function isMultiArray($multiarray)
    {
